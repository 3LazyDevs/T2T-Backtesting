...
def run(info):
    ...
    ind_open = [float(ele) for ele in pd.Series.tolist(ind_history['Open']) ]# open values
    ind_high = [float(ele) for ele in pd.Series.tolist(ind_history['High']) ]# high values
    ind_low = [float(ele) for ele in pd.Series.tolist(ind_history['Low'])]#low values
    ind_close = [float(ele) for ele in pd.Series.tolist(ind_history['Close'])]# close values
